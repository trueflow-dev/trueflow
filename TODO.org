...
** TODO [High] Basic Infrastructure
   - [ ] Define `trueflow-mode` and basic buffer setup.
   - [ ] Implement `trueflow.el` wrapper for CLI calls (`trueflow diff`, `trueflow mark`, `trueflow sync`).
   - [ ] Define customization groups (executable path, faces).

** TODO [High] Status View (`trueflow-status`)
   - [ ] Render "Unreviewed Blocks" section.
   - [ ] Implement `trueflow-refresh` to parse JSON from CLI.
...
** TODO [Medium] Iterative Review Mode
   - [ ] Command `trueflow-review-start` (bound to `r`).
   - [ ] Walk through blocks one by one.
...
* CLI Improvements (trueflow CLI)
** TODO [Medium] `trueflow check`
   - [ ] Implement CI gate logic (exit 1 if unreviewed blocks exist).
