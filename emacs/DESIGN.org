#+title: Trueflow Emacs UX Design
#+DATE: 2026-01-18
#+STARTUP: content hidestars

* Emacs User Experience (UX)

** 1. Entry Point: `trueflow-status`
   - Invoked via `M-x trueflow` or `M-x trueflow-status`.
   - **Auto-Sync:** Runs `trueflow sync` asynchronously on startup to ensure latest state.
   - **Buffer:** `*trueflow-status*` (derived from `magit-section-mode`).
   - **Content:**
     - Header: "Trueflow Status: [Repo Name]"
...
** 2. Review Mode (Iterative Flow)
   ...
   - **Layout:** Reuses `*trueflow-status*` buffer but narrows/zooms to the current item, or uses a dedicated Transient layout.
   - **View:** 
...
** 4. Handling Stale Data
   - **Scenario:** User edits file outside of Trueflow.
   - **Reaction:** `trueflow-refresh` runs (hooked to `after-save-hook` in project files? or manual `g`).
   - **Missing Hunks:** If a hunk fingerprint is no longer found in `trueflow diff` output, it is silently dropped from the UI.

** 5. Package Structure
   - Follows MELPA conventions.
   - `trueflow.el`: Main entry point and mode definitions.
   - `trueflow-core.el`: Process wrappers for `trueflow` CLI.
   - `trueflow-ui.el`: Rendering logic (Magit sections).
