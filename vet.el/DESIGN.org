#+title: Vet Emacs UX Design
#+DATE: 2026-01-18
#+STARTUP: content hidestars

* Emacs User Experience (UX)

** 1. Entry Point: `vet-status`
   - Invoked via `M-x vet` or `M-x vet-status`.
   - **Auto-Sync:** Runs `vet sync` asynchronously on startup to ensure latest state.
   - **Buffer:** `*vet-status*` (derived from `magit-section-mode`).
   - **Content:**
     - Header: "Vet Status: [Repo Name]"
...
** 2. Review Mode (Iterative Flow)
   ...
   - **Layout:** Reuses `*vet-status*` buffer but narrows/zooms to the current item, or uses a dedicated Transient layout.
   - **View:** 
...
** 4. Handling Stale Data
   - **Scenario:** User edits file outside of Vet.
   - **Reaction:** `vet-refresh` runs (hooked to `after-save-hook` in project files? or manual `g`).
   - **Missing Hunks:** If a hunk fingerprint is no longer found in `vet diff` output, it is silently dropped from the UI.

** 5. Package Structure
   - Follows MELPA conventions.
   - `vet.el`: Main entry point and mode definitions.
   - `vet-core.el`: Process wrappers for `vet` CLI.
   - `vet-ui.el`: Rendering logic (Magit sections).
