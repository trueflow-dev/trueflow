...
use crate::store::{FileStore, Identity, Record, RepoRef, ReviewStore, ReviewTargetRef};
use crate::targets::{
    ResolvedTargets, ReviewContentSource, ReviewDiffTarget, ReviewPathSelection, ReviewTarget,
...
struct FeedbackCursor {
    timestamp: i64,
    record_ids_at_timestamp: Vec<String>,
}
...
struct FeedbackCollectionOptions<'a> {
    ...
    apply_changed_path_selection: bool,
}
...
pub fn run(
    ...
    let entries = collect_feedback_entries(
        ...
        &FeedbackCollectionOptions {
            ...
            include_approved,
            workdir_prefix: workdir_prefix_from_git_root().as_deref(),
            scan_options: &scan_options,
            apply_changed_path_selection: targets
                .iter()
...
fn collect_feedback_entries(
    ...
    let FeedbackCollectionOptions {
        ...
        workdir_prefix,
        scan_options,
        apply_changed_path_selection,
    } = *options;
    let explicit_selection = targets.explicit_selection();
    let changed_selection = apply_changed_path_selection
        .then(|| targets.changed_selection())
        .flatten();
    let latest_verdicts = latest_verdicts_by_target(database.records());
    let allowed_revisions = resolve_allowed_revisions(&targets.diff_selection)?;
    let default_snapshot = snapshot_from_content_source(&targets.content_source);
    ...
    let filtered_records = database
        .records()
        .iter()
        .filter(|record| record_matches_since(record, since_filter))
        .filter(|record| record_matches_allowed_revisions(record, allowed_revisions.as_ref()))
        .collect::<Vec<_>>();
    ...
    for record in filtered_records {
        ...
        if file_path != "<unknown>" && should_skip_imports_by_default(&file_path, &block, filters) {
            continue;
        }
...
fn resolve_allowed_revisions(
    diff_selection: &crate::targets::ReviewDiffSelection,
...
fn build_feedback_cursor(records: &[Record]) -> Option<FeedbackCursor> {
...
fn read_feedback_cursor(path: &Path) -> Result<Option<FeedbackCursor>> {
...
fn write_feedback_cursor(path: &Path, cursor: &FeedbackCursor) -> Result<()> {
        ...
        let records = vec![
            Record {
                ...
                target: ReviewTargetRef::Block {
                    hash: crate::hashing::TreeHash::new("one"),
                },
                check: crate::store::ReviewCheck::review(),
                verdict: crate::store::Verdict::Comment,
                identity: crate::store::Identity::Email {
                    email: "a@example.com".to_string(),
                },
                repo_ref: crate::store::RepoRef::Unknown,
                block_state: crate::store::BlockState::Unknown,
                timestamp: 10,
                path_hint: None,
                line_hint: None,
                note: None,
            ...
            Record {
                ...
                version: 2,
                target: ReviewTargetRef::Block {
                    hash: crate::hashing::TreeHash::new("two"),
                },
                check: crate::store::ReviewCheck::review(),
                verdict: crate::store::Verdict::Comment,
                identity: crate::store::Identity::Email {
            ...
            Record {
                ...
                version: 2,
                target: ReviewTargetRef::Block {
                    hash: crate::hashing::TreeHash::new("three"),
                },
                check: crate::store::ReviewCheck::review(),
                verdict: crate::store::Verdict::Comment,
                identity: crate::store::Identity::Email {
