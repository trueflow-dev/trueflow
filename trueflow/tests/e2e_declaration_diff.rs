...
fn single_unit(diff: &DeclarationDiff) -> Result<&DeclarationDiffUnit> {
    diff.units
        ...
        .with_context(|| format!("expected exactly one declaration diff unit, got {:?}", diff.units))
}
...
fn body_only_edit_matches_but_creates_no_review_unit() -> Result<()> {
    ...
    assert_eq!(diff.matches.len(), 1, "the unchanged surface must still match");
...
fn documentation_signature_field_and_variant_edits_are_exact_changed_units() -> Result<()> {
    ...
    for case in cases {
        ...
        let base = unit.base.as_ref().context("changed unit must retain its base projection")?;
        let head = unit.head.as_ref().context("changed unit must retain its head projection")?;

        assert_eq!(unit.change_kind, DeclarationChangeKind::Changed, "{}", case.id);
        assert_eq!(base.kind, case.expected_kind, "{} base kind", case.id);
        assert_eq!(head.kind, case.expected_kind, "{} head kind", case.id);
        assert_eq!(base.projection_text, case.expected_base_projection, "{} base", case.id);
        assert_eq!(head.projection_text, case.expected_head_projection, "{} head", case.id);
        assert_ne!(base.projection_hash, head.projection_hash, "{} must reopen", case.id);
        assert!(diff.diagnostics.is_empty(), "{}: {:?}", case.id, diff.diagnostics);
    }
...
fn additions_and_deletions_retain_the_only_available_exact_projection() -> Result<()> {
    ...
    assert_eq!(
        deleted.base.as_ref().context("deleted base")?.projection_text,
        "pub struct Removed {\n    pub reason: String,\n}"
    );
...
fn same_name_overloads_match_by_signature_without_cross_pairing() -> Result<()> {
    ...
    assert_eq!(
        integer.head.as_ref().context("integer head")?.projection_text,
...
fn ambiguous_duplicate_declarations_are_not_arbitrarily_paired() -> Result<()> {
    ...
    assert!(diff.matches.is_empty(), "an ambiguous candidate must not be guessed");
    assert_eq!(
        diff.units
            .iter()
...
fn declaration_name_change_reopens_a_uniquely_matched_declaration() -> Result<()> {
    ...
    assert_eq!(unit.matching_evidence, Some(MatchingEvidence::UniqueNameElided));
    assert_eq!(
...
fn equal_hash_units_from_distinct_snapshot_pairs_are_never_deduplicated() -> Result<()> {
    ...
    assert_eq!(
        diff.units[0].head.as_ref().context("first head")?.projection_hash,
        diff.units[1].head.as_ref().context("second head")?.projection_hash,
