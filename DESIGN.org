...
* 1. Overview
  **Trueflow** (historically called Anchor / Vibecheck) is a semantic code review system. It decouples review status
...
** How do we handle identical blocks (boilerplate)?
   ...
   - *Override:* If they require separate context, the ~path_hint~ metadata helps the reviewer
     identify the location. Future iterations could add a ~unique_salt~ to the review-target identifier
