...
- Diff mode computes textual diff fingerprints in `trueflow/src/diff_logic.rs`.
...
Key Gaps:
- Two identity systems do not interoperate cleanly: content-addressed block review targets (whose current CLI/API field name is still `fingerprint`) vs separate diff fingerprints. This can lead to inconsistent review outcomes.
...
- Unify identity: map diff fingerprints to block/content hashes, or move diff logic to block-level changes.
